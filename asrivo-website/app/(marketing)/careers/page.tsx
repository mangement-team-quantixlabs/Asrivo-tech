      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-brand-midnight via-brand-navy to-brand-midnight pt-32 pb-20">
        ...
        <div className="relative z-[1] mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex flex-wrap gap-6">
            {[{ icon: Briefcase, val: "25+", label: "Open Positions" }, { icon: MapPin, val: "8", label: "Global Offices" }, { icon: Users, val: "200+", label: "Team Members" }].map(({ icon: Icon, val, label }) => (
              <div key={label} className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/10"><Icon className="h-4 w-4 text-white/80" /></div>
                <div><div className="text-lg font-bold text-white">{val}</div><div className="text-xs text-white/50">{label}</div></div>
              </div>
            ))}
          </div>
        </div>
