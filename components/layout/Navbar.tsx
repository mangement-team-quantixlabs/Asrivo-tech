    ...
    >
      <nav className="mx-auto flex h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <span
            className={cn(
              "text-xl font-bold tracking-tight transition-colors font-[var(--font-sora)]",
        ...
        <div className="hidden lg:flex items-center gap-3">
          <Link
            href="/contact"
            className="inline-flex h-10 items-center gap-2 rounded-full bg-brand-orange px-5 text-sm font-semibold text-white transition-all hover:bg-brand-orange/90 hover:shadow-lg hover:shadow-brand-orange/25 active:scale-[0.98]"
          >
            Talk to an Expert
          ...
          >
            <div className="mx-auto max-w-7xl space-y-1 px-4 pb-6 pt-2 sm:px-6">
              ...
              <div className="pt-4">
                <Link
                  href="/contact"
                  className="flex h-11 w-full items-center justify-center gap-2 rounded-full bg-brand-orange text-sm font-semibold text-white transition-all hover:bg-brand-orange/90"
                >
                  Talk to an Expert
                  <ArrowRight className="h-4 w-4" />
                </Link>
    ...
    return (
      ...
      >
        ...
        {isActive && (
          <motion.div
            layoutId="nav-active"
            className="absolute -bottom-0.5 left-3 right-3 h-0.5 rounded-full bg-brand-electric"
          />
        )}
      </Link>
    );
              ...
              <div className="grid grid-cols-2 gap-1">
                {link.children!.map((child) => {
                  ...
                  return (
                    ...
                    >
                      {Icon && (
                        <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-navy/5 text-brand-navy transition-colors group-hover:bg-brand-navy group-hover:text-white">
                          <Icon className="h-4.5 w-4.5" />
                        </div>
                      )}
                      <div>
                        <div className="text-sm font-semibold text-brand-navy group-hover:text-brand-electric">
                          {child.label}
                        </div>
                        {child.description && (
...
function MobileNavItem({
  ...
  if (!hasChildren) {
    return (
      <Link
        href={link.href}
        className="flex h-11 items-center rounded-lg px-3 text-base font-medium text-brand-gray-700 hover:bg-brand-gray-100 hover:text-brand-navy"
      >
        {link.label}
      </Link>
    );
              ...
              {link.children!.map((child) => {
                ...
                return (
                  <Link
                    key={child.href}
                    href={child.href}
                    className="flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm text-brand-gray-700 hover:bg-brand-gray-100 hover:text-brand-navy"
                  >
